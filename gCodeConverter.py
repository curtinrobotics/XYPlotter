...
def create_shape(shapeName, shapeDataDict):
...
for item in fileListStrip:
    if item != "":
        ...
        if objName in ATTRIBUTE_LIST:
            ...
            shapeObjList = create_shape(objName, objDict)
                    
            
            #Attribute(objName, )
            #shapeObjList.append(object)
        elif objName in FORMAT_SYSTAX:
            print(objName + " format")
        elif objName == "g":
            # Creates g container
            print("Open g container")
            objGData = get_obj_data(item)
            print(objGData)
